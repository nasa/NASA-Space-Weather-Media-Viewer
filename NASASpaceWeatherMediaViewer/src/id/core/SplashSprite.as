package id.core
{
	...
	public class TouchSprite extends SpriteVisualElement
	{
		...
		public static var TAP:String = "tap has been thrown";
		public static var OUT_OF_BOUNDS:String="out of bounds";
		...
		private function onTapDown(event:TouchEvent):void
		{
			...
			dispatchEvent(new Event(TouchSprite.TAP));
			
			//trace("tap down inside")
		}
		...
		private function onTouchDown(event:TouchEvent):void
		{
			...
			if (pointArray.length>1)
			{
				...
				var dox:Number=pointArray[0].point.x;
				var doy:Number=pointArray[0].point.y;
				var dox2:Number=pointArray[1].point.x;
				var doy2:Number=pointArray[1].point.y;
				...
				var value:Number=Math.atan2(Y,X)*(180/Math.PI);
				...
				if (hasEventListener(gestureList[i].@type))
				{
					...
					for (var j:int=0; j<gestureList[i].TouchPoints.point.length(); j++)
					{
						if (count==gestureList[i].TouchPoints.point[j])
						{
							...
							var func:Object=functionDictionary[gestureList[i].calculation.@type.toString()]();
		...
		private function checkObjectBounds():void
		{
			if(((x+width)-(width*.10))<0 || x>GestureWorks.application.stageWidth-(width*.10) || ((y+height)-(width*.10))<0 || y>GestureWorks.application.stageHeight-(width*.10))
			{
				...
				dispatchEvent(new Event(TouchSprite.OUT_OF_BOUNDS));
			}
		}
		...
		private function scaleValue():Number
		{
			...
			return currentScale+((widthDiff*percWid)/100);
		}
